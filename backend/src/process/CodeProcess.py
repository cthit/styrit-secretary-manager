...
from queries.GroupMeeting import get_group_meeting_by_code, get_last_upload_for_code, get_meeting_for_code, \
...
def handle_code_request(code_str: str) -> HttpResponse:
    ...
    return get_with_data({
        "code": code_str,
        "data": get_data_for_code(code_res.data)
    })


def get_data_for_code(code: uuid) -> Optional[Dict]:
